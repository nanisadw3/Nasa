...
# NOTA: Clave puesta directamente para facilitar la ejecución.
API_KEY = "wRkfHugxxWLwBz5TnAZ2uFQyK7mmDgMT3HcbEQ8j"
...
def apod():
    ...
    if cache_entry["data"] and (time.time() - cache_entry["timestamp"] < APOD_CACHE_DURATION_SECONDS):
    ...
    try:
        ...
        if post.get('media_type') == 'video' and post.get('url') and 'youtube.com' in post['url']:
...
def posts():
    ...
    # Si hay datos en caché y no han expirado, los usamos
    if cache_entry["data"] and (time.time() - cache_entry["timestamp"] < POSTS_CACHE_DURATION_SECONDS):
    ...
    if isinstance(respuesta, dict) and respuesta.get('error'):
        return f"<h1>Error de la API de la NASA</h1><p>{respuesta['error']['message']}</p>"
